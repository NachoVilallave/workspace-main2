...
st.title("Aplicación predicción meteorológica")
st.write("Obtenga el pronóstico del tiempo para cualquier ubicación utilizando la API Open-Meteo.")
...
with col1:
    lat = st.number_input("Itroducir Latitud:", value=0.0, format="%.6f", step=0.000001)
with col2:
    lon = st.number_input("Introducir Longitud:", value=0.0, format="%.6f", step=0.000001)

fetch_button = st.button("Obtener Predicción")


# STEP6: Validamos lo que introduce el usuario
if lat < -90 or lat > 90:
    st.error("Latitud debe estar entre -90 y 90.")
if lon < -180 or lon > 180:
    st.error("Longitud debe estar entre -180 y 180.")
...
if fetch_button and (-90 <= lat <= 90) and (-180 <= lon <= 180):
    ...
    if data:
        ...
        st.write(f"Pronóstico del tiempo para Latitud: {lat}, Longitud: {lon}")
        ...
        fig = px.line(forecast_df, x='Tiempo', y='Temperatura (°C)', title='Predicción temperatura')
...
#export PATH="/home/nacvilvil/.local/bin/:$PATH" <-- abajo en el recuadro del terminal
